...
class EdofNet(nn.Module):
    def __init__(self, max_dilation):
        ...
        self._stride = 1
        self.input_layer = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, stride=1, padding=1,
                                                   bias=False), nn.BatchNorm2d(32, momentum=0.05))
        ...
        for dilation in self.dilations:
            self.hidden_blocks.append(self.hidden_block(dilation))
    ...
    @staticmethod
    def hidden_block(dilation):
        return nn.Sequential(nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=dilation,
                                       dilation=dilation, bias=False), nn.BatchNorm2d(32))
